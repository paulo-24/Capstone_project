...
<body>
                                                ...
                                                <div class="col">
                                                    <div class="d-flex align-items-center mb-3">
                                                        ...
                                                        <div class="ms-auto" aria-hidden="true">
                                                            <?php if (!empty($rubricRequirements)) { ?>
                                                                <button type="button" class="btn btn-warning" data-bs-toggle="modal" data-bs-target="#courseRequirementsModal">
                                                                    Edit Requirements
                                                                </button>
                                                            <?php } ?>
                                                        </div>
                                                    ...
                                                    <?php } else { ?>
                                                        <div class="alert alert-warning text-center" role="alert">
                                                            <p>Your course requirements are empty. Please fill them up!</p>
                                                            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#courseRequirementsModal">
                                                                Add Requirements
                                                            </button>
                                                        </div>
                                                    <?php } ?>
    ...
    <tbody>
        ...
        foreach ($submissions as $submission) {
            ...
            $student_name = $student_info['fullName'] ?? 'Unknown';
            $score = !empty($submission['score']) ? $submission['score'] : 'N/A';
            $submission_date = $submission['submission_date'] ? $submission['submission_date'] : 'N/A';
            $status = $submission['status'] ?: 'pending';
            $hasSubmission = $submission_date !== 'N/A' && $submission['status'] !== 'none'; // Check if submission exists
        ?>
            <tr data-submission-id="<?php echo $submission['id']; ?>">
                ...
                <td><?php echo htmlspecialchars(ucfirst($submission['type'])); ?></td>
                <td>
                    <span class="score-placeholder">
                        <input type="number" 
                               class="form-control score-input" 
                               value="<?php echo htmlspecialchars($score === 'N/A' ? '' : $score); ?>" 
                               data-submission-id="<?php echo $submission['id']; ?>" 
                               <?php echo $hasSubmission ? '' : 'disabled'; ?> 
                               min="0" 
                ...
                <td>
                    ...
                    echo $formatted_due_date->format('F j, Y') . ' at ' . $formatted_due_time->format('g:i A');
                    ?>
                </td>
                <td>
                    <?php
                    if ($submission_date && strtotime($submission_date)) {
                        ...
                        echo $formatted_date->format('F j, Y');
                    } else {
                ...
                <td class="status-cell">
                    ...
                    ?>
                    <span class="badge <?php echo $status_class; ?> text-light"><?php echo htmlspecialchars($status_text); ?></span>
                </td>
                <td>
                    <?php if ($hasSubmission) { ?>
                        <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#viewGradeModal<?php echo $submission['id'] ?>"><i class="bi bi-eye-fill"></i> View</button>
                        <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#editGradeModal<?php echo $submission['id'] ?>"><i class="bi bi-pen-fill"></i> Edit</button>
                    <?php } else { ?>
                        <button class="btn btn-warning" onclick="notifyToPassActivity(<?php echo $submission['id']; ?>, <?php echo $submission['student_id']; ?>)"><i class="bi bi-exclamation-diamond-fill"></i> Notify</button>
                    <?php } ?>
                </td>
            </tr>

            <!-- View and Edit modals remain unchanged -->
            <!-- ... Include your existing modal code here ... -->
        <?php
        }
        ?>
    </tbody>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    scoreInputs.forEach(input => {
        input.addEventListener('change', function() {
            ...
            // Validate score
            if (newScore === '' || isNaN(newScore) || newScore < 0 || newScore > maxPoints) {
                Swal.fire('Error', `Score must be between 0 and ${maxPoints}`, 'error');
                this.value = ''; // Reset to previous valid value or empty
                return;
            }

            // Send AJAX request to update the score
            updateScore(submissionId, newScore);
        });
    });

    function updateScore(submissionId, score) {
        const data = {
            submission_id: submissionId,
            score: score,
            status: 'graded', // Automatically set to graded on score change
            class_id: '<?php echo $id; ?>' // Ensure $id is available from PHP
        };
        ...
        .then(result => {
            if (result.success) {
                ...
                statusCell.className = 'badge bg-success text-light';
                statusCell.textContent = 'Graded';

                Swal.fire('Success', 'Score updated successfully', 'success');
            } else {
                Swal.fire('Error', result.error || 'Failed to update score', 'error');
            }
        })
        .catch(error => {
            console.error('Error updating score:', error);
            Swal.fire('Error', 'An error occurred while updating the score', 'error');
        });
    }

    // Existing notifyToPassActivity function remains unchanged
            ...
            Swal.fire({
                ...
                confirmButtonText: 'Try Again'
                                                            ...
                                                            <script>
                                                                ...
                                                                function applyFilters() {
                                                                    // Get filter values
                                                                    const studentFilter = document.getElementById('studentFilter').value.toLowerCase().trim();
                                                                    const activityTypeFilter = document.getElementById('activityTypeFilter').value;
                                                                    const activityNameFilter = document.getElementById('activityNameFilter').value.toLowerCase().trim();
                                                                    ...
                                                                    rows.forEach(row => {
                                                                        // Extract row data
                                                                        const studentName = row.cells[0].textContent.toLowerCase().trim();
                                                                        const activityName = row.cells[1].textContent.toLowerCase().trim();
                                                                        const activityType = row.cells[2].textContent.trim();
                                                                        ...
                                                                        // Handle date comparison: Convert to Date objects
                                                                    ...
                                                                    const allFiltersCleared = !studentFilter && !activityTypeFilter && !activityNameFilter && !dateFilter && !statusFilter;
             ...
             WHERE se.class_id = :class_id2"
                    ...
                    <tbody>";
                                                                                ...
                                                                                echo "<div class='button-group text-center'><br><br>";
                                                                                // Disable Revert and Save buttons if all grades are accepted
                                                                                echo "<button type='submit' name='action' value='revert' class='btn btn-secondary' style='margin-right:5px' " . ($canRevert && !$allAccepted ? "" : "disabled") . ">Revert Grades</button>";
                                                                                echo "<button type='submit' name='action' value='save' class='btn btn-success' style='margin-right:5px' " . (($hasSaved || $hasForApproval || $allAccepted) ? "disabled" : "") . ">Save Grades</button>";
                                                                                if ($hasForApproval) {
                                                                                    echo "<button type='submit' name='action' value='submit' class='btn btn-warning' disabled>Submit Grades</button>";
                                                                                } else {
                                                                                    echo "<button type='submit' name='action' value='submit' class='btn btn-warning' " . ($allAccepted ? "disabled" : "") . ">Submit Grades to Dean</button>";
                                                                                }
                                                                                echo "</div>";
                                                                ...
                                                                <div class="d-flex align-items-center">
                                                                    ...
                                                                    <div class="ms-auto" aria-hidden="true">
                                                                        <button class="btn btn-primary mb-4" data-bs-toggle="modal"
                                                                            ...
                                                                            <i class="bi bi-plus-circle-fill"></i> Upload Learning
                                                                            Resource
                                                                        </button>
                                                                    </div>
                                                                </div>
