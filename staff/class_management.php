                                        ...
                                        <div class="card">
                                            ...
                                            try {
                                                ...
                                                if (!$classes) {
                                                    ...
                                                    echo '<h5 class="card-title">Today’s Classes</h5>';
                    ...
                    ");
                                                        ...
                                                        if (!empty($todays_classes)) {
                                                            ...
                                                            foreach ($todays_classes as $class) {
                                                                echo '<li>'
                                                                    . htmlspecialchars($class['subject_name']) . ' - ' // Use the subject_name
                                    ...
                                    <div class="col">
                                        <div class="card">
                                            ...
                                            try {
                                                ...
                                                $classes_stmt = $pdo->prepare("SELECT id, status FROM classes WHERE teacher = :teacher_name AND status != 'pending'");
                                                $classes_stmt->execute(['teacher_name' => $teacher_name]);
                                                $classes = $classes_stmt->fetchAll(PDO::FETCH_ASSOC);

                                                // Count the number of active classes the teacher handles
                                                $classes_handling = count($classes);

                                                if ($classes_handling === 0) {
                                                    // No active classes assigned to the teacher
                                                    echo '<div class="card-body">';
                                                    echo '<h5 class="card-title">Attendance Rate</h5>';
                                                    echo '<p>You haven\'t been assigned to any active classes, yet!</p>';
                                                    echo '</div>';
                                                } else {
                                                    // Initialize variables to calculate the average attendance rate
                                                    $total_classes = 0;
                                                    $total_present = 0;

                                                    // Get the meeting_id from 'classes_meetings' for each active class and calculate attendance
                                                    foreach ($classes as $class) {
                                                        $class_id = $class['id'];

                                                        // Fetch the meeting IDs for the current class
                                                        ...
                                                        foreach ($meetings as $meeting) {
                                                            ...
                                                            // Count total attendance entries for this meeting
                                                            ...
                                                            // Count the "present" status for this meeting
                                                    ...
                                                    // Calculate the average attendance rate as a percentage
                                                    $average_attendance_rate = $total_classes > 0 ? ($total_present / $total_classes) * 100 : 0;

                                                    // Display the Attendance Summary
                                                    echo '<div class="card-body">';
                                                    echo '<h5 class="card-title">Attendance Summary</h5>';
                                                    echo '<p>Classes Handling: ' . htmlspecialchars($classes_handling) . '</p>';

                                                    if ($total_classes === 0) {
                                                        echo '<p>No attendance records found for your classes yet.</p>';
                                                    } else {
                                                        echo '<p>Average Attendance Rate: ' . number_format($average_attendance_rate, 2) . '%</p>';
                                                    }
                                                ...
                                                echo "An error occurred while fetching the attendance summary.";
                                            }
                                            ?>
                                        </div>
                                    </div>
